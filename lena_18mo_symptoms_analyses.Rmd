---
title: "lena_18mo_symptoms_analyses"
author: "Fran Querdasi"
date: "3/2/2019"
output: html_document
---

```{r setup, include=FALSE}
#Parameters

##set up CRAN
r = getOption("repos")
r["CRAN"] = "http://cran.us.r-project.org"
options(repos = r)

#load libraries
library(tidyverse)
library("Hmisc")
library(car)
library(corrr)
library("corrplot")
library("lme4")
library(ggpubr)

#get files
crisys_file <- "~/Desktop/lena_18mo_symptoms/Data/crisys_6mo_20190202.csv"
q_18mo_file <- "~/Desktop/lena_18mo_symptoms/Data/q_18month_scores_20190202.csv"
q_18mo_ages_file <- "~/Desktop/lena_18mo_symptoms/Data/q_18month_ages.csv"
demographics_file <- "~/Desktop/lena_18mo_symptoms/Data/demographics_20190131.csv"
lena_file <- "~/Desktop/lena_18mo_symptoms/Data/lena_cleaned_day1_20190202.csv"
lena_follow_and_data_file <- "~/Desktop/lena_18mo_symptoms/Data/lena_followup_and_data_20190202.csv"
baby_raceth_file <- "~/Desktop/lena_18mo_symptoms/Data/baby_race_ethnicity.csv"
  
```
#Read in and join data
```{r}
lena_18mo_sym_df <-
  read_csv(q_18mo_file) %>% 
  select(
    ID, 
    itsea_symptoms,
    itsea_competence
  ) %>% 
  left_join(
    read_csv(q_18mo_ages_file) %>% 
      select(
        ID,
        baby_age_18month,
        baby_age_18month_int
      )
  ) %>% 
  left_join(
    read_csv(crisys_file) 
  ) %>% 
  left_join(
    read_csv(demographics_file) %>% 
      select(
        ID,
        education,
        annual_income,
        income_needs,
        male,
        mom_age_6mo_visit,
        mom_race,
        mom_ethnicity
      )
  ) %>% 
  left_join(
    read_csv(baby_raceth_file) %>% 
      select(
        -childrace_describe
      )
  ) %>% 
  left_join(
    read_csv(lena_file) %>% 
      rename(
        baby_age_lena = age
      )
  )

lena_18mo_sym_df <-
  lena_18mo_sym_df %>% 
  filter(
    !is.na(day),
    !(ID %in% c(1019, 1020) & is.na(mom_age_6mo_visit)) #these were duplicates
    ) %>% 
  unique()  

glimpse(lena_18mo_sym_df)
  
```
#demographics in only subset with complete data
```{r}
lena_18mo_sym_df_complete <-
  lena_18mo_sym_df %>% 
  filter(
    complete.cases(lena_18mo_sym_df)
  )

#mean and sd of continuous variables
demo_summaries_complete <-
lena_18mo_sym_df_complete %>% 
  summarize_at(
    vars(
    baby_age_18month,
    baby_age_lena,
    duration,
    mom_age_6mo_visit
    ), 
    list(~mean, ~sd, ~min, ~max),
    na.rm = TRUE)

#counts and percentages for categorical variables - education, ethnicity, race, income
lena_18mo_sym_df_complete %>% 
  count(annual_income) %>% 
  mutate(per = n / sum(n)) %>% 
  arrange(desc(annual_income))

lena_18mo_sym_df_complete %>% 
  count(income_needs < 1) %>% 
  mutate(per = n / sum(n))

lena_18mo_sym_df_complete %>% 
  count(mom_ethnicity) %>% 
  mutate(per = n / sum(n))

lena_18mo_sym_df_complete %>% 
  count(child_ethnicity) %>% 
  mutate(per = n / sum(n))
  
lena_18mo_sym_df_complete %>% 
  count(mom_race) %>% 
  mutate(per = n / sum(n)) %>% 
  arrange(desc(per))

lena_18mo_sym_df %>% 
  count(child_race) %>% 
  mutate(per = n / sum(n)) %>% 
  arrange(desc(per))

lena_18mo_sym_df_complete %>% 
  count(education) %>% 
  mutate(per = n / sum(n)) %>% 
  arrange(desc(per))
```

#look at distributions of data
```{r}
qplot(lena_18mo_sym_df$itsea_competence, 
      geom = "histogram",
      binwidth = .05)

qplot(lena_18mo_sym_df$itsea_symptoms, 
      geom = "histogram",
      binwidth = .05)

qplot(lena_18mo_sym_df$baby_age_18month, 
      geom = "histogram",
      binwidth = .5)

qplot(lena_18mo_sym_df$crisys_total_6mo, 
      geom = "histogram",
      binwidth = .5)

qplot(lena_18mo_sym_df$income_needs, 
      geom = "histogram",
      binwidth = .1)

qplot(lena_18mo_sym_df$education, 
      geom = "histogram",
      binwidth = .1)

qplot(lena_18mo_sym_df$baby_age_lena, 
      geom = "histogram",
      binwidth = .1)

qplot(lena_18mo_sym_df$duration, 
      geom = "histogram",
      binwidth = .5)

qplot(lena_18mo_sym_df$awc_prop, 
      geom = "histogram",
      binwidth = .05)

qplot(lena_18mo_sym_df$ctc_prop, 
      geom = "histogram",
      binwidth = .05)

qplot(lena_18mo_sym_df$cvc_prop, 
      geom = "histogram",
      binwidth = .05)

count(lena_18mo_sym_df, day_type)
count(lena_18mo_sym_df_complete, day_type)

```
#Look at means, std, min, max
```{r}
summaries_continuous <-
lena_18mo_sym_df %>% 
  summarise_at(
    vars(
      itsea_competence, 
      itsea_symptoms, 
      crisys_total_6mo_win, 
      crisys_total_6mo,
      awc_prop, 
      ctc_prop, 
      cvc_prop
      ), 
    funs(
        mean, 
        sd, 
        min, 
        max), 
    na.rm = TRUE)

summaries_continuous <-
lena_18mo_sym_df_complete %>% 
  summarise_at(
    vars(
      itsea_competence, 
      itsea_symptoms, 
      crisys_total_6mo_win, 
      crisys_total_6mo,
      awc_prop, 
      ctc_prop, 
      cvc_prop
      ), 
    list(
        ~mean, 
        ~sd, 
        ~min, 
        ~max,
        ~median), 
    na.rm = TRUE)

#is there a way to reformat this data table?
```
#correlation tables
```{r}
#lucy's correlation function
correlations_continuous <-
  lena_18mo_sym_df %>% 
  select(
    itsea_symptoms,
    itsea_competence,
    baby_age_18month,
    baby_age_lena,
    mom_age_6mo_visit,
    income_needs,
    crisys_total_6mo_win,
    duration,
    awc_daily_rate,
    ctc_daily_rate,
    cvc_daily_rate,
    awc_prop,
    ctc_prop,
    cvc_prop
  )

corr_table <- correlate(correlations_continuous)

corr_mat <- rcorr(as.matrix(correlations_continuous), type = c("pearson"))
corr_mat

# flattenCorrMatrix <- function(cormat, pmat) {
#   ut <- upper.tri(cormat)
#   data.frame(
#     row = rownames(cormat)[row(cormat)[ut]],
#     column = rownames(cormat)[col(cormat)[ut]],
#     cor  =(cormat)[ut],
#     p = pmat[ut]
#     )
# }
# 
# flattenCorrMatrix(corr_mat$r, corr_mat$p)

#Insignificant correlations are left blank
corrplot(corr_mat$r, type="upper", order="hclust", 
         p.mat = corr_mat$P, sig.level = 0.05, insig = "blank")


```
#center continuous variables
```{r}
    #create variable that is median split of crisys, and also a factor
lena_18mo_sym_df <-
  lena_18mo_sym_df %>% 
  mutate(
    crisys_median_split = if_else(
      crisys_total_6mo_win > median(crisys_total_6mo_win, na.rm = TRUE),
      "stressed", "not stressed"
      )
  )

lena_18mo_sym_df <-
  lena_18mo_sym_df %>% 
  mutate(
    crisys_median_split_num = if_else(
      crisys_median_split == "stressed", 
      1, 0
    ),
    crisys_median_factor = as.factor(crisys_median_split_num)
  )

contrasts(lena_18mo_sym_df$crisys_median_factor) = c(-1,1)

lena_18mo_sym_df <-
  lena_18mo_sym_df %>% 
  mutate(
    awc_prop_centered = scale(awc_prop, scale = FALSE),
    itsea_symptoms_centered = scale(itsea_symptoms, scale = FALSE),
    ctc_prop_centered = scale(ctc_prop, scale = FALSE),
    cvc_prop_centered = scale(cvc_prop, scale = FALSE),
    itsea_competence_centered = scale(itsea_competence, scale = FALSE)
  )
```
#associations among categorical variables
```{r}
##(categorical variables = crisys_median_split, education, male, day_type)

##Is education related to daily rates, props, symptoms, competence?

#make education a factor
lena_18mo_sym_df <-
  lena_18mo_sym_df %>% 
  mutate(education_factor = as.factor(education))

contrasts(lena_18mo_sym_df$education_factor) = cbind(c(0,1,0), c(0,0,1)) #lowest ed group is baseline

#1. Daily Rates:
education_level_awc_daily <- lm(
  awc_daily_rate ~ 
    education_factor,
  data = lena_18mo_sym_df
)
summary(education_level_awc_daily)
#what do the "factor 1" and "factor 2" variables mean here?
Anova(education_level_awc_daily, type = 3) #used Anova from car rather than anova because anova doesn't have "type" argument

#RESULT: ns, p = 0.07

education_level_ctc_daily <- lm(
  ctc_daily_rate ~ 
    education_factor,
  data = lena_18mo_sym_df
)
summary(education_level_ctc_daily)
Anova(education_level_ctc_daily, type = 3)
#RESULT: sig, sum square = 1058.2, df = 2, F = 3.9194, p = .026
#INTERP: higher edu -- higher daily rate of ctc?

education_level_cvc_daily <- lm(
  cvc_daily_rate ~ 
    education_factor,
  data = lena_18mo_sym_df
)
summary(education_level_cvc_daily)
Anova(education_level_cvc_daily, type = 3)
#RESULT: sig
#INTERP: higher edu = higher daily rate of cvc?

#2. Props

education_level_awc_prop <- lm(
  awc_prop ~ 
    education_factor,
  data = lena_18mo_sym_df
)
summary(education_level_awc_prop)
Anova(education_level_awc_prop, type = 3)
#RESULT: intercept sig, but education factor ns

education_level_ctc_prop <- lm(
  ctc_prop ~ 
    education_factor,
  data = lena_18mo_sym_df
)
summary(education_level_ctc_prop)
Anova(education_level_ctc_prop, type = 3)
#RESULT: sig
#INTERP: 

#didn't work
ggboxplot(lena_18mo_sym_df, x = "education_factor", y = "ctc_prop", 
          color = "education_factor", palette = c("#00AFBB", "#E7B800", "#FC4E07"),
          order = c("some college", "BA", "graduate degree"),
          ylab = "ctc_prop", xlab = "Education Level")

education_level_cvc_prop <- lm(
  cvc_prop ~ 
    education_factor,
  data = lena_18mo_sym_df
)
summary(education_level_cvc_prop)
Anova(education_level_cvc_prop, type = 3)
#RESULT: intercept sig, rest not
#INTERP: 

#3. ITSEA
education_level_itsea_sx <- lm(
  itsea_symptoms ~ 
    education_factor,
  data = lena_18mo_sym_df
)
summary(education_level_itsea_sx)
Anova(education_level_itsea_sx, type = 3)
#RESULT: intercept sig, rest not
#INTERP

education_level_itsea_comp <- lm(
  itsea_competence ~ 
    education_factor,
  data = lena_18mo_sym_df
)
summary(education_level_itsea_comp)
Anova(education_level_itsea_comp, type = 3)
#RESULT: intercept sig, rest not
#INTERP

##Is day_type related to daily rates, props, symptoms, competence?

lena_18mo_sym_df %>% 
  count(day_type)
#41 weekdays, 16 weekends

t.test(lena_18mo_sym_df$awc_daily_rate ~ lena_18mo_sym_df$day_type)
#no difference
t.test(lena_18mo_sym_df$ctc_daily_rate ~ lena_18mo_sym_df$day_type)
#no difference
t.test(lena_18mo_sym_df$cvc_daily_rate ~ lena_18mo_sym_df$day_type)
#higher cvc daily rate in weekday vs. weekend group, t(31.644) = 2.2196, p=.03374
t.test(lena_18mo_sym_df$awc_prop ~ lena_18mo_sym_df$day_type)
#ns
t.test(lena_18mo_sym_df$ctc_prop ~ lena_18mo_sym_df$day_type)
#ns
t.test(lena_18mo_sym_df$cvc_prop ~ lena_18mo_sym_df$day_type)
#ns
t.test(lena_18mo_sym_df$itsea_symptoms ~ lena_18mo_sym_df$day_type)
#ns
t.test(lena_18mo_sym_df$itsea_competence ~ lena_18mo_sym_df$day_type)
#ns

#differences among stressed vs. non-stressed groups by education, income_needs, daily rates, props, itsea_symptoms, itsea_competence

chisq.test(lena_18mo_sym_df$crisys_median_split, lena_18mo_sym_df$education_factor)
#"may be incorrect" - small cell size for 1 level of education

t.test(lena_18mo_sym_df$income_needs ~ lena_18mo_sym_df$crisys_median_split)
#ns, higher in not stressed
t.test(lena_18mo_sym_df$awc_daily_rate ~ lena_18mo_sym_df$crisys_median_split)
#ns, higher in stressed
t.test(lena_18mo_sym_df$ctc_daily_rate ~ lena_18mo_sym_df$crisys_median_split)
#ns
t.test(lena_18mo_sym_df$cvc_daily_rate ~ lena_18mo_sym_df$crisys_median_split)
#ns
t.test(lena_18mo_sym_df$awc_prop ~ lena_18mo_sym_df$crisys_median_split)
#ns
t.test(lena_18mo_sym_df$ctc_prop ~ lena_18mo_sym_df$crisys_median_split)
#ns
t.test(lena_18mo_sym_df$cvc_prop ~ lena_18mo_sym_df$crisys_median_split)
#ns
t.test(lena_18mo_sym_df$itsea_symptoms ~ lena_18mo_sym_df$crisys_median_split)
#higher in stressed group, t(37.844) = -2.1569, p = .037
t.test(lena_18mo_sym_df$itsea_competence ~ lena_18mo_sym_df$crisys_median_split)
#ns

```
#demographics
```{r}
#mean and sd of continuous variables
demo_summaries <-
lena_18mo_sym_df %>% 
  summarize_at(
    vars(
    baby_age_18month,
    baby_age_lena,
    duration,
    mom_age_6mo_visit
    ), 
    list(~mean, ~sd, ~min, ~max),
    na.rm = TRUE)

#counts and percentages for categorical variables - education, ethnicity, race, income
lena_18mo_sym_df %>% 
  count(annual_income) %>% 
  mutate(per = n / sum(n)) %>% 
  arrange(desc(annual_income))

lena_18mo_sym_df %>% 
  count(income_needs < 1) %>% 
  mutate(per = n / sum(n))

lena_18mo_sym_df %>% 
  count(mom_ethnicity) %>% 
  mutate(per = n / sum(n))

lena_18mo_sym_df %>% 
  count(child_ethnicity) %>% 
  mutate(per = n / sum(n))
  
lena_18mo_sym_df %>% 
  count(mom_race) %>% 
  mutate(per = n / sum(n)) %>% 
  arrange(desc(per))

lena_18mo_sym_df %>% 
  count(child_race) %>% 
  mutate(per = n / sum(n)) %>% 
  arrange(desc(per))

lena_18mo_sym_df %>% 
  count(education) %>% 
  mutate(per = n / sum(n)) %>% 
  arrange(desc(per))
```

#Run main analyses
```{r}
#don't need to specify intercept
#F-statistic - whether overall model is sig
#obs = higher df + 1 
#Adjusted R-squared - how much variance model explains

lena_18mo_sym_int_awc <- 
  lm(
    itsea_symptoms_centered ~ 
      crisys_total_6mo_win * awc_prop_centered,
    data = lena_18mo_sym_df 
                 )

summary(lena_18mo_sym_int_awc)
#significant, adjusted R squared = .1482

lena_18mo_sym_int_ctc <- 
  lm(
    itsea_symptoms ~ 
      crisys_total_6mo_win * ctc_prop,
    data = lena_18mo_sym_df)

summary(lena_18mo_sym_int_ctc)
#ns

lena_18mo_sym_int_cvc <- 
  lm(
    itsea_symptoms ~ 
      crisys_total_6mo_win * cvc_prop,
    data = lena_18mo_sym_df)
summary(lena_18mo_sym_int_cvc)
#ns

#explanation of summary output: 
#Residuals: Residuals are essentially the difference between the actual observed response values (distance to stop dist in our case) and the response values that the model predicted. The Residuals section of the model output breaks it down into 5 summary points. When assessing how well the model fit the data, you should look for a symmetrical distribution across these points on the mean value zero (0).
#Coefficents: represent intercept and slope terms in model.
#coefficient standard error: The coefficient Standard Error measures the average amount that the coefficient estimates vary from the actual average value of our response variable. We’d ideally want a lower number relative to its coefficients.
#coefficient t value: The coefficient t-value is a measure of how many standard deviations our coefficient estimate is far away from 0. We want it to be far away from zero as this would indicate we could reject the null hypothesis
#Resdiual standard error: Residual Standard Error is measure of the quality of a linear regression fit. Theoretically, every linear model is assumed to contain an error term E. Due to the presence of this error term, we are not capable of perfectly predicting our response variable  from the predictor one. The Residual Standard Error is the average amount that the response  will deviate from the true regression line.
#R squared: The R-squared (R2) statistic provides a measure of how well the model is fitting the actual data. It takes the form of a proportion of variance. R2 is a measure of the linear relationship between our predictor variable and our response / target variable. It always lies between 0 and 1
#F-statistic is a good indicator of whether there is a relationship between our predictor and the response variables. The further the F-statistic is from 1 the better it is. However, how much larger the F-statistic needs to be depends on both the number of data points and the number of predictors.
```
#main effects
```{r}
lena_18mo_sym_awc_cent <- 
  lm(
    itsea_symptoms_centered ~ 
      awc_prop_centered,
    data = lena_18mo_sym_df 
                 )

summary(lena_18mo_sym_awc_cent)
#

lena_18mo_sym_awc <- 
  lm(
    itsea_symptoms_centered ~ 
      awc_prop,
    data = lena_18mo_sym_df 
                 )

summary(lena_18mo_sym_awc)
#awc_prop estimate = -.3767, t = -1.343, p = .185

lena_18mo_sym_ctc <- 
  lm(
    itsea_symptoms ~ 
      ctc_prop,
    data = lena_18mo_sym_df)

summary(lena_18mo_sym_ctc)
#ctc_prop estimate = -0.5743, t = -2.39, p = .02

lena_18mo_sym_cvc <- 
  lm(
    itsea_symptoms ~ 
      cvc_prop,
    data = lena_18mo_sym_df)
summary(lena_18mo_sym_cvc)
#cvc_prop estimate = -0.5256, t = -1.71, p = .093

lena_18mo_sym_df %>%
  ggplot(aes(ctc_prop, itsea_symptoms)) +
  geom_point() +
  geom_smooth(method ="lm")
```


#run model with covs
```{r}
lena_18mo_sym_int_sex_cov <- 
  lm(itsea_symptoms ~ 
       crisys_total_6mo_win * awc_prop_centered 
     + male, 
     data = lena_18mo_sym_df)
summary(lena_18mo_sym_int_sex_cov)

lena_18mo_sym_int_awc_covs <- 
  lm(itsea_symptoms ~ 
       crisys_total_6mo_win * awc_prop 
     + baby_age_lena 
     + male 
     + baby_age_18month,
     data = lena_18mo_sym_df)
summary(lena_18mo_sym_int_awc_covs)
#explains even more variance!
```
#test other potential predictors (that were correlated in the correlation matrix)
```{r}
lena_18mo_sym_int_awc_daily <- 
  lm(
    itsea_symptoms_centered ~ 
      crisys_total_6mo_win * awc_daily_rate,
    data = lena_18mo_sym_df 
                 )
summary(lena_18mo_sym_int_awc_daily)
#entire model sig (p = 0.028), but none of the individual predictors are

lena_18mo_sym_int_ctc_daily <- 
  lm(
    itsea_symptoms_centered ~ 
      crisys_total_6mo_win * ctc_daily_rate,
    data = lena_18mo_sym_df 
                 )
summary(lena_18mo_sym_int_ctc_daily)
#entire model sig (p = 0.023), but none of the individual predictors are

lena_18mo_sym_int_cvc_daily <- 
  lm(
    itsea_symptoms_centered ~ 
      crisys_total_6mo_win * cvc_daily_rate,
    data = lena_18mo_sym_df 
                 )
summary(lena_18mo_sym_int_cvc_daily)
#ns

lena_18mo_sym_int_cvc <- 
  lm(
    itsea_symptoms_centered ~ 
      crisys_total_6mo_win * cvc_prop,
    data = lena_18mo_sym_df 
                 )
summary(lena_18mo_sym_int_cvc)
#ns

lena_18mo_sym_int_ctc <- 
  lm(
    itsea_symptoms_centered ~ 
      crisys_total_6mo_win * ctc_prop,
    data = lena_18mo_sym_df 
                 )
summary(lena_18mo_sym_int_ctc)
#entire model sig (p = .0271), but none of individual predictors
```

#determine adjusted r squared for just interaction term
#run each predictor separately
```{r}
#amt of varianc explained just by crisys
lena_18mo_sym_just_crisys <- 
  lm(
    itsea_symptoms_centered ~ 
      crisys_total_6mo_win,
    data = lena_18mo_sym_df 
                 )
summary(lena_18mo_sym_just_crisys)
#adjusted r squared: 0.0264; ns

#amt of variance explained just by awc_prop_centered
lena_18mo_sym_just_awc_prop <- 
  lm(
    itsea_symptoms_centered ~ 
     awc_prop_centered,
    data = lena_18mo_sym_df 
                 )
summary(lena_18mo_sym_just_awc_prop)
#adjusted r squared: 0.0126, ns

#full model w interaction adjusted r squared = .1482, so just interaction term explains:
int_adjust_r_squared = .1419 - .0264 - .0126
print(int_adjust_r_squared)
#variance explained = .1092
```
#test awc_daily_rate
```{r}
lena_18mo_sym_int_awc_rate <- 
  lm(
    itsea_symptoms ~ 
      crisys_total_6mo_win * awc_daily_rate,
    data = lena_18mo_sym_df, 
    contrasts = (contrasts(lena_18mo_sym_df$crisys_median_factor) = c(-1,1)
                 )
    )
summary(lena_18mo_sym_int_awc_rate)
```


#run model with just weekdays and see if results hold
```{r}
lena_18mo_sym_df_weekdays <-
  lena_18mo_sym_df %>% 
  filter(
    day_type == "weekday"
  )

lena_18mo_sym_int_awc_weekdays <- 
  lm(
    itsea_symptoms ~ 
      crisys_total_6mo_win * awc_prop,
    data = lena_18mo_sym_df_weekdays, 
    contrasts = (contrasts(lena_18mo_sym_df$crisys_median_factor) = c(-1,1)
                 )
  )
summary(lena_18mo_sym_int_awc_weekdays)
```

#visualize interactions
```{r}

count(lena_18mo_sym_df, crisys_median_split)

lena_18mo_sym_df %>%
  filter(
    !is.na(crisys_median_split)
  ) %>% 
  ggplot(aes(awc_prop, itsea_symptoms, color = crisys_median_split)) +
  geom_point() +
  geom_smooth(method ="lm")

lena_18mo_sym_df %>%
  filter(
    !is.na(crisys_median_split)
  ) %>% 
  ggplot(aes(ctc_prop, itsea_symptoms, color = crisys_median_split)) +
  geom_point() +
  geom_smooth(method ="lm")
```
#examine LENA follow-up data
```{r}
#Within all ps with followup data and awc_prop
lena_followup_and_data <-
  read_csv(lena_follow_and_data_file)

#see if percent_mother associated with awc_depend_prop
lena_momtime_awc <- 
  lm(
    awc_prop ~ percent_mother,
    data = lena_followup_and_data 
    )
summary(lena_momtime_awc)
#association not significant (p = .69)

#cor.test (use spearman's correlation if data is weirdly distributed to check correlation)

#visualize regression
lena_followup_and_data %>% 
ggplot(aes(percent_mother, awc_prop)) +
  geom_point() +
  geom_smooth(method ="lm")

#report stats on percent_mother
lena_followup_and_data %>% 
  summarize_at(
    vars(
    percent_mother
    ), 
    list(~mean, ~sd, ~min, ~max),
    na.rm = TRUE)

#Within just subset of ps using for this project
 
lena_follow_complete_data <-
  lena_18mo_sym_df_complete %>% 
  left_join(
    read_csv(lena_follow_and_data_file),
    by = "ID"
  )

lena_follow_complete_data %>% 
  summarize_at(
    vars(
    percent_mother
    ), 
    list(~mean, ~sd, ~min, ~max),
    na.rm = TRUE) 

lena_follow_complete_data %>% 
  count(percent_mother) %>% 
  mutate(total = sum(!is.na(n)))

#see if percent_mother associated with weekday vs weekend
t.test(lena_follow_complete_data$percent_mother ~ lena_follow_complete_data$day_type.x)
  
```


#Regression diagnostics
```{r}
#assessing outliers
outlierTest(lena_18mo_sym_int_awc) # Bonferonni p-value for most extreme obs
#not sure why this didn't work
qqPlot(lena_18mo_sym_int_awc, main="QQ Plot") #qq plot for studentized resid 
leveragePlots(lena_18mo_sym_int_awc) # leverage plots

#calculate standardized estimates
#library(sjstats)
#no covs model
#std_beta(lena_18mo_sym_int_awc)
#covs model
#std_beta(lena_18mo_sym_int_awc_covs)

# Normality of Residuals
# qq plot for studentized resid
qqPlot(lena_18mo_sym_int_awc, main="QQ Plot")
# distribution of studentized residuals
# library(MASS)
# sresid <- studres(lena_18mo_sym_int_awc) 
# hist(lena_18mo_sym_int_awc, freq=FALSE, 
#    main="Distribution of Studentized Residuals") #error - x must be numeric
# xfit<-seq(min(sresid),max(sresid),length=40) 
# yfit<-dnorm(xfit) 
# lines(xfit, yfit)

#conclusions about qqplot: 
#leverage plots: 

# Evaluate Collinearity
vif(lena_18mo_sym_int_awc) # variance inflation factors 
sqrt(vif(lena_18mo_sym_int_awc)) > 2 # problem?
#may be problem (crisys and interaction)

# Global test of model assumptions
library(gvlma)
gvmodel <- gvlma(lena_18mo_sym_int_awc) 
summary(lena_18mo_sym_int_awc)
```
#probe interaction w simple slope analysis
```{r}
#create new variables: high and low awc_prop_centered
lena_18mo_sym_df <-
  lena_18mo_sym_df %>% 
  mutate(
    awc_prop_hi = scale(awc_prop, scale = F) - sd(awc_prop),
    awc_prop_low = scale(awc_prop, scale = F) + sd(awc_prop)
  )

#run model everything centered again (just for kicks)
lena_18mo_sym_int_awc <- 
  lm(
    itsea_symptoms_centered ~ 
      crisys_total_6mo_win * awc_prop_centered,
    data = lena_18mo_sym_df 
                 )

summary(lena_18mo_sym_int_awc)


#run model with awc_prop_hi
lena_18mo_sym_int_awc_hi <- 
  lm(
    itsea_symptoms_centered ~ 
      crisys_total_6mo_win * awc_prop_hi,
    data = lena_18mo_sym_df 
                 )

summary(lena_18mo_sym_int_awc_hi)
#when dependability is high, crisys variable beta is -.004198 and it is not significant
#so when dep is high, greater past 6 month stress predicts slightly lower sx

#run model with awc_prop_low
lena_18mo_sym_int_awc_low <- 
  lm(
    itsea_symptoms_centered ~ 
      crisys_total_6mo_win * awc_prop_low,
    data = lena_18mo_sym_df 
                 )

summary(lena_18mo_sym_int_awc_low)
#when dependability is low, crisys variable beta is .02966 and it is significant
#so when dep is low, greater past 6 month stress predicts greater symptoms

```

#olsrr diagnostics, standardized betas 
```{r}
##standardized betas

library(olsrr)
ols_regress_model_middle <-
ols_regress(itsea_symptoms ~ 
       crisys_total_6mo_win * awc_prop_centered,
       data = lena_18mo_sym_df)
print(ols_regress_model_middle)
#ols_plot_diagnostics(lena_18mo_sym_int_awc) - not really sure why this isn't working
 #standardized betas: interaction term: -.341 (-.317, -.044); awc_prop_centered: -.150 (-.287, 1.655); crisys_total_6mo_win: .237 (-.001, .027)

ols_regress_model_high <-
ols_regress(itsea_symptoms ~ 
       crisys_total_6mo_win * awc_prop_hi,
       data = lena_18mo_sym_df)
print(ols_regress_model_high)
 
ols_regress_model_low <-
ols_regress(itsea_symptoms ~ 
       crisys_total_6mo_win * awc_prop_low,
       data = lena_18mo_sym_df)
print(ols_regress_model_low)


##olsrr diagnostics
plot(lena_18mo_sym_int_awc)
#residuals vs. fitted should be straight line
#normal q-q should follow line
#scale location shows if residuals are spread equally along the ranges of predictors. This is how you can check the assumption of equal variance (homoscedasticity). It’s good if you see a horizontal line with equally (randomly) spread points.
#residuals vs leverage: allows identification of potentially influential outliers to the model. Influential ones are outside cook's distance. 

```
