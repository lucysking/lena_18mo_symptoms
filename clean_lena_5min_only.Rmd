...
lena_5min_cl <- 
  ...
  summarise(
    ...
    ctc_speech = sum(CTC.Actual >= 1),
    cvc_speech = sum(CVC.Actual >= 1)
  ) %>% 
  mutate(
    awc_daily_rate = awc_speech/duration, #daily AWC rate
    ctc_daily_rate = ctc_speech/duration, #daily CTC rate
    cvc_daily_rate = cvc_speech/duration, #daily CVC rate
