...
q_18month_cs_file <- "~/Desktop/lena_18mo_symptoms/Data/questionnaires_18mo_cs_20190202.csv"
q_18month_long_file <- "~/Desktop/lena_18mo_symptoms/Data/questionnaires_18mo_long_20190202.csv"
```
...
x %>% 
  filter(!is.na(awc_prop_D1)) %>% 
  ggplot(aes(sixmonth_crisys_win, itsea_f1_symptoms, color = awc_prop_D1 > median(awc_prop_D1, na.rm = TRUE))) +
  geom_point(size = 6) +
  geom_smooth(method = "lm", se = FALSE, size = 4) +
  scale_x_continuous(breaks = seq.int(0, 15, 3)) +
  #scale_y_continuous(breaks = seq.int(.19, .30, .01)) +
  scale_color_manual(
    labels = c("Lower dependability", "Higher dependability"),
    values = c("#B42F32", "#494940")
  ) +
  theme_minimal() +
  theme(
  ...
  labs(
    color = "",
    x = "Sustained threat: Life stress",
    y = "18-month negative emotionality "
  ) 

ggsave("~/Desktop/BABIES/r01_winter_2019/plots/crisys_symp_awcprop.png", width = 9, height = 8)




summary(lm(itsea_f1_symptoms ~ awc_prop_D1 * sixmonth_crisys_win, data = x))

x %>% 
  filter(!is.na(maltreated)) %>% 
  ggplot(aes(sixmonth_crisys_win, itsea_f1_symptoms, color = maltreated)) +
  geom_point(size = 6) +
  geom_smooth(method = "lm", se = FALSE, size = 4) +
  scale_x_continuous(breaks = seq.int(0, 15, 3)) +
  #scale_y_continuous(breaks = seq.int(.19, .30, .01)) +
  scale_color_discrete(
    labels = c("Mother maltreated", "No maltreatment")
  ) +
  theme_minimal() +
  theme(
...
ggsave("~/Desktop/BABIES/r01_winter_2019/plots/crisys_symp_maltr.png", width = 7, height = 6)

t.test(x$awc_prop_D1 ~ x$maltreated)
```
