...
lena_5min_cl_wf <-
  ...
  distinct(ID, date_record_D1, ctc_daily_rate_D1, .keep_all = TRUE) %>% 
  group_by(ID) %>% 
  mutate(
...
```{r}
#IDs to use day 2: 
# day 1: 110 (put device in pocket rather than chest day 1)
...
lena_time_cl <-
  ...
  mutate(
    ...
    time_day = case_when(
      time_hours >= ct_first_time & time_hours <= 10 ~ "CT_AM",
      time_hours >= 10 & time_hours < 13 ~ "CT_MID",
      time_hours >= 13 & time_hours < 17 ~ "CT_AFT",
      time_hours >= 17 & time_hours < ct_last_time ~ "CT_PM"
    )
  ) %>% 
...
``` {r}
# calculate CT rate during each phase of day 
lena_time_day <- 
  lena_time_cl %>% 
...
lena_time_wf <- 
  ...
  spread(time_day, ctc_time_rate, sep = "_")
...
lena_d1 <-
  lena_d1 %>% 
  distinct(ID, ctc_daily_rate, .keep_all = TRUE) %>% 
...
identify_outliers_hist(lena_d1, time_day_AM)
identify_outliers_hist(lena_d1, time_day_MID)
identify_outliers_hist(lena_d1, time_day_AFT)
identify_outliers_hist(lena_d1, time_day_PM)
```
...
```{r}
write_csv(lena_d1, "~/Desktop/BABIES/lena_symptoms/data_final/lena_d1_20190904.csv")
write_csv(lena_time_cl, "~/Desktop/BABIES/lena_symptoms/data_final/lena_d1_time_20190904.csv")
```