...
## Calculate CTC based on time of day
```{r}
lena_time <- 
  lena_5min %>% 
  group_by(ID) %>% 
...
ggsave("~/Desktop/BABIES/lena_symptoms/manuscript/ctc_time.png")
```
...
lena_5min_cl <-
  lena_time_day %>% 
  left_join(lena_5min_cl, by = c("ID", "age", "date_record")) %>% 
  filter(
    (age >= 5 & age <= 8),
    duration >= 8
  ) %>% 
...
lena_5min_cl_wf <-
  ...
  distinct(ID, date_record_D1, ctc_time_rate_D1, .keep_all = TRUE) %>% 
  group_by(ID) %>% 
  mutate(
...
```{r}
#IDs to use day 2: 
# day 1: 110 (put device in pocket rather than chest day 1), 106
