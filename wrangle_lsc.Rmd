...
NOTE: Prior to cleaning in R, ages of events were cleaned as follows to reflect age of onsent (raw data files remain saved on mooddata):

- Use the earliest age of the event as the value. For example, if a participant reported witnessing an accident at ages 5, 10, 24 years, the age value is "5". 

- If participant reported a range of ages (e.g., 10-15), use the earliest age. 

- If participant reported "all my life" or "all of childhood", the age value is "0".
...
stress_ratings_cs_file1 <- "~/Desktop/BABIES/lena_symptoms/data/cs6_LSCR_2018-11_12_18_rated_n76_cleaned.xlsx"

stress_ratings_cs_file2 <- "~/Desktop/BABIES/lena_symptoms/data/cs6_LSCR_2019-04_08_19_rated_clean.xlsx"

stress_ratings_long_file <- "~/Desktop/BABIES/lena_symptoms/data/long_preg_LSCR_2018-10-01_rated.xlsx"

dates_file <- "~/Desktop/BABIES/lena_symptoms/data/demo_lscr_20190412.csv"
...
dates <-
  ...
  mutate(
    ...
    mom_age_preg_visit = (mom_dob %--% preg_date) / years(1),
    mom_age_6mo_visit = (mom_dob %--% sixmonth_date) / years(1)
  )
```
...
```{r}
lscr_6mo <- 
  readxl::read_xlsx(stress_ratings_cs_file1) %>% 
  rename(ID = record_id) %>% 
  select(
    -contains("date")
  ) %>% 
  bind_rows(
    readxl::read_xlsx(stress_ratings_cs_file2) %>% 
      rename(ID = record_id) %>% 
...
lscr_T1lg_6mocs <- 
  lscr_T1lg_6mocs %>% 
  mutate(
    sum_sev_child_win = winsorize(x = sum_sev_childhood, product = 3),
    sum_sev_precon_win = winsorize(x = sum_sev_precon, product = 3),
    num_stress_total_win = winsorize(x = num_stress_total, product = 3),
    sum_sev_win = winsorize(x = sum_sev, product = 3),
    
  )
```

##Examine merged data distributions
```{r}
lscr_T1lg_6mocs %>% 
  ggplot(aes(num_stress_total)) +
  geom_histogram(binwidth = 1) +
